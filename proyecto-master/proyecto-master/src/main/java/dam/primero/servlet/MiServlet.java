...
public class MiServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String pathInfo = request.getServletPath(); // puede ser "/registrar.html" o "/login"
		System.out.println("En get:"+pathInfo				);
		if (pathInfo == null || pathInfo.trim().isEmpty() || pathInfo.trim().equalsIgnoreCase("/login") || pathInfo.trim().equalsIgnoreCase("/login.html")) {
		    templateEngine.process("login", context, response.getWriter());
		} else {
		    String accion = pathInfo.substring(1);
		    if (accion.endsWith(".html")) {
		        accion = accion.substring(0, accion.length() - 5);  // eliminar ".html"
		    }
